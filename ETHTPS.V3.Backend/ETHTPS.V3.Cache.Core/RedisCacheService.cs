...
namespace ETHTPS.V3.Cache.Core
{
    public sealed class RedisCacheService : ISyncCache, IAsyncCache, IDisposable
    {
        ...
        public void SetBigDictionary<T>(string key, IDictionary<string, T> value)
        {
            var batch = _database.CreateBatch();
            foreach (var group in value.Chunk(500))
            {
                var hashEntries = group.Select(kv => new HashEntry(kv.Key, JsonConvert.SerializeObject(kv.Value))).ToArray();
